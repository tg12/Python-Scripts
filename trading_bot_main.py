...
#Work in fiat, because thats what we want after all, right??
 ...
 if amount_in_gbp_fiat > starting_price_fiat:
  ...
  if percentage < -3:
   ...
   open('/root/MyPythonCode/tradingbot.log', 'a+').write("Percentage Value was " + str(percentage) + '\n')
   ...
   #DO THE SELL!
   sell = client.sell(acc_id,total=float(price_change),currency="GBP") #Is this a float or a string??? Not sure check this out later
   #GET THE SELL ID!
   sell_id = str(sell.id)
   ...
   time.sleep(3) #Wait a token 3 seconds for price to update before reading it
   ...
   print ("DEBUG: NO PROFIT OPPORTUNITY!!! Price is neither above 1.6% or below -3% to BTFD")
   open('/root/MyPythonCode/tradingbot.log', 'a+').write(str(datetime.datetime.now()) + " DEBUG: NO PROFIT OPPORTUNITY!!! Price is neither above 1.6% or below -3% to BTFD" + '\n')
