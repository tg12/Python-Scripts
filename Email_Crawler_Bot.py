...
try:
    ...
    def getmails(link, proxy):
        ...
        try:
            ...
            if proxy == '--proxy':
                dataOfLink = urllib.urlopen(
                    link, proxies={'http': randomip}).read()
            else:
                dataOfLink = urllib.urlopen(link).read()
            mailsfound = re.findall(
                '"\"?([-a-zA-Z0-9.`?{}]+@\w+\.\w+)\"?"', dataOfLink)
    ...
    def pastebin(prox):
        ...
        try:
            randomip = proxies[randint(0, len(proxies) - 1)]
            if prox == '--proxy':
                getdata = urllib.urlopen(
                    pburl, proxies={
                        'http': randomip}).read()
            else:
                getdata = urllib.urlopen(pburl).read()
            pasteurl = re.findall(
                'class="i_p0" alt="" /><a href="(.*?)">', getdata)
    ...
    def debpaste(prox):
        ...
        try:
            ...
            if prox == '--proxy':
                ...
                getdata = urllib.urlopen(pburl).read()
            pasteurl = re.findall(
                "<li><a href='//paste.debian.net(.*?)'>", getdata)
            i = 1
            for link in pasteurl:
    ...
    def searx(dork, pages, prox):
        ...
        while p <= m:
            data = urllib.urlencode(
                {'category_general': '1', 'q': dork, 'pageno': p, 'time_range': '', 'language': 'all'})
            data = data.replace('+', '%20')
            try:
                search = urllib2.Request(
                    'https://searx.laquadrature.net/', data)
